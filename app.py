...
def play_speech():
    ...
    if speech_text:
        tts = gTTS(text=speech_text, lang=language+"-IN", slow=False)
        ...
        audio_path = os.path.join(audio_directory, 'speech.mp3')

        # Remove older files
        files = glob.glob(os.path.join(audio_directory, '*.mp3'))
        for old_file in files:
            os.remove(old_file)
            print(f"Old audio file removed: {old_file}")

        # Save the new audio stream to a file
        with open(audio_path, 'wb') as audio_file:
            audio_file.write(audio_stream.read())
        print(f"Processed answer saved as {audio_path}")
        ...
        response.headers['Content-Disposition'] = f'inline; filename=speech_{timestamp}.mp3'
